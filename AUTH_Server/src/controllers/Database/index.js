const { User } = require("sequelize_models");
const bcrypt = require("bcrypt");
const crypto = require('crypto');
const { promisify } = require('util');

const { ERR } = require("messages");
const { setError } = ERR;

async function addUserToDB(req, _, next) {

  const rawBytes = await promisify(crypto.randomBytes)(16);
  const imageName = rawBytes.toString('hex');
  console.log('\n\nKEY', imageName, '\n\n')

  try {
    await User.addNewUser({
      email: req.session.email,
      password: req.session.password, name: req.body.name,
      surname: req.body.surname, phone: req.body.phone,
      profile_image_key: imageName
    });
    console.log("New user added to DB");
    next();
  } catch (error) {
    console.log("Error adding new user in sequelize : ", error);
    next(setError("ERR_NETWORK"));
  }
}

async function verifyLogin(req, _, next) {

  const { email, password } = req.body;

  try {
    const user = await User.findByEmail(email);

    if (!user) {
      throw setError("USER_DOESNT_EXIST");
    }
    console.info("\n\nComparing passwords : ");
    let plainUser = user.get({ plain: true });
    const match = await bcrypt.compare(password, plainUser.password);
    if (!match) {
      throw setError("WRONG_LOGIN_CREDENTIALS");
    }
    console.log("Password match")
    delete plainUser.password;
    console.log("user : ", plainUser);
    req.body.user = plainUser;
    next();
  } catch (error) {
    console.log("Error logging in user  : ", error);
    next(error);
  }
}

const updateProfileImage = async (req, _, next) => {
  console.log("updateProfileImage : ", req.body)
  const profile_image = req.body.url;
  const userId = req.session.userId;

  try {
    const user = await User.updateUser(userId, { profile_image });

    // const user = await User.findById(userId);
    console.log("User profile image updated : ", user);
    req.body.user = user;
    next();
  } catch (error) {
    console.log("Error updating user profile image : ", error);
    next(error);
  }

};


const deleteProfileImage = async (req, _, next) => {
  const profile_image = null;
  const userId = req.session.userId;

  try {
    const user = await User.updateUser(userId, { profile_image });
    console.log("user : ", "!!!!!!!!!!!!!!!!!!!")
    console.log("User profile image updated : ", user);
    req.body.user = user;
    next();
  } catch (error) {
    console.log("Error updating user profile image : ", error);
    next(error);
  }

};

module.exports = { addUserToDB, verifyLogin, updateProfileImage, deleteProfileImage };
